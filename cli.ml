...
let parse () =
  let args_list =
    Arg.align
      [
        ("-ad",   Arg.Unit
                  (fun () -> opt.allow_dir <- true ),
                  " allow dir to be moved too." );
        ...
        ("-dn",   Arg.Unit
                  (fun () -> opt.dn        <- true ),
                  " prepend current dirname to file / dir" );

        ("-ty",   Arg.Unit
                  (fun () -> opt.year      <- true ),
                  " time year:    use file's last modification time as YYYY." );
        ("-tmon", Arg.Unit
                  (fun () -> opt.month     <- true ),
                  " time month:   use file's last modification time as YYYY-MM." );

        ("-td",   Arg.Unit
                  (fun () -> opt.day       <- true ),
                  " time day:     use file's last modification time as YYYY-MM-DD" );

        ("-th",   Arg.Unit
                  (fun () -> opt.hour      <- true ),
                  " time hours:   use file's last modification time as YYYY-MM-DD_hh'h'" );

        ("-tm",   Arg.Unit
                  (fun () -> opt.min       <- true ),
                  " time minutes: use file's last modification time as YYYY-MM-DD_hh'h'mm'm'" );

        ("-ts",   Arg.Unit
                  (fun () -> opt.sec       <- true ),
                  " time seconds: use file's last modification time as YYYY-MM-DD_hh'h'mm'm'ss's'" );

        ("-tf",   Arg.Unit
                  (fun () -> opt.usefloat  <- true ),
                  " time float:   use file's last modification time since unix epoche as float value (%f)" );
  ...
  ("-rnmode",  Arg.String
        ...
        *)
        ...
        ("-gitcmd", Arg.Unit
                    (fun ()   -> opt.gitcmd   <- true         ),
                    " no move/rename, instead print 'git mv' commands - dirs will still be created" );
      ]
  in
    Arg.parse args_list
